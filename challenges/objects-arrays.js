...
for (const person of graduates) {
  universities.push(person.university);
}
...
for (const person of graduates) {
  const val = person.first_name + " " + person.email;
...
function getPopSum(total, animal) {
  return total + animal.population;
}

const populationTotal = zooAnimals.reduce(getPopSum, 0);
